...
namespace Udger.Parser
{
    ...
    public static readonly String SQL_CLIENT_CH_REGEX =
            "SELECT " +
                "class_id," +
                "client_id," +
                "regstring," +
            ...
            "FROM udger_client_ch_regex " +
                ...
                "JOIN udger_client_class ON udger_client_class.id= udger_client_list.class_id " +
                    "WHERE udger_client_ch_regex.mobile = {0} ORDER BY sequence ASC";

        public static readonly String SQL_OS_CH_REGEX =
            "SELECT " +
                "os_id,regstring,family,family_code,name,name_code,homepage,icon,icon_big,vendor,vendor_code,vendor_homepage " +
                "FROM udger_os_ch_regex " +
                "JOIN udger_os_list ON udger_os_list.id=udger_os_ch_regex.os_id " +
                "WHERE " +
                @"udger_os_ch_regex.version = '{0}' OR udger_os_ch_regex.version = '-all-' " +
                "ORDER BY sequence ASC";

        public static readonly String SQL_DEVICENAME_CH_REGEX =
                        "SELECT id FROM udger_devicename_regex WHERE " +
                            "((os_family_code= '{0}' AND os_code = '-all-') " +
                            "OR " +
                            "(os_family_code= '{1}' AND os_code = '{2}')) " +
                            "order by sequence " +
                            "LIMIT 0,1 ";

        public static readonly String SQL_DEVICENAMELIST_CH =
            "SELECT marketname,brand_code,brand,brand_url,icon,icon_big,deviceclass_id " +
            "FROM udger_devicename_list " +
            "JOIN udger_devicename_brand ON udger_devicename_brand.id=udger_devicename_list.brand_id " +
            "WHERE regex_id='{0}' and code = '{1}' ";

        public static readonly String SQL_DEVICECLASSLIST_CH =
            "SELECT name,name_code,icon,icon_big " +
            "FROM udger_deviceclass_list " +
            "WHERE id = '{0}'";

        public static readonly String SQL_DEVICECLASSLISTMOBILE_CH =
            "SELECT name,name_code,icon,icon_big " +
            "FROM udger_deviceclass_list " +
            "JOIN udger_deviceclass_ch ON udger_deviceclass_list.id=udger_deviceclass_ch.device_id " +
            "WHERE udger_deviceclass_ch.mobile = '{0}'";
